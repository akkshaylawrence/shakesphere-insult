...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Three classic Shakespearean insult columns.
    const col1Words = [
    ...
    const insultOutput = document.getElementById('insult-output');
    ...
    function renderInsult() {
        insultOutput.textContent = `“${buildInsult()}”`;
    }

    function generateInsult() {
        currentWords = [
            pickRandom(getPool(col1Words)),
            pickRandom(getPool(col2Words)),
            pickRandom(getPool(col3Words))
        ];
        renderInsult();
    }
    ...
    copyButton.addEventListener('click', async () => {
        try {
            await navigator.clipboard.writeText(buildInsult());
            showStatus('Copied to clipboard.');
        } catch {
            showStatus('Could not copy. Select the insult and copy it manually.');
        }
    });
